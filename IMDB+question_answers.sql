...
-- By using count command, find out total rows in each table.
...
-- Type your code below:
-- By using COUNT(column name) we will get total rows having information.
...
SELECT Count(title) AS movies_total
FROM   movie
...
SELECT SUM(genre_count) AS movie_with_one_genre
FROM genre_unique_summary;
...
SELECT genre, 
       ...
       RANK() OVER w1   AS genre_rank
FROM   movie AS m
	   INNER JOIN genre AS g
			ON m.id = g.movie_id
GROUP  BY genre
WINDOW w1 AS (ORDER BY COUNT(title) DESC)
)
SELECT *
...
WITH top_rating_movies AS
(
SELECT title,
	   avg_rating,
       ROW_NUMBER() OVER w1 AS movie_rank
FROM   movie AS m
...
SELECT 	CASE
			...
			ELSE 'others'
		END AS language,
        SUM(total_votes) AS movies_votes
FROM    movie AS m
			INNER JOIN ratings AS r
				ON m.id = r.movie_id
...
SELECT language, SUM(movies_votes) AS total_votes
FROM language_movie_votes
GROUP  BY language; 
...
FROM   director_mapping AS d
            ...
            INNER JOIN ratings AS r
				ON r.movie_id = m.id
			INNER JOIN Top_Three_genre AS t
				USING (genre)
WHERE      avg_rating > 8
...
SELECT name AS actor_name,
	   SUM(total_votes) AS total_votess,
       COUNT(title) AS movie_count,
       ROUND(SUM(avg_rating * total_votes) / SUM(total_votes), 3) AS actor_avg_rating,
       ROW_NUMBER() OVER w1 AS actor_rank
FROM   names AS n
			INNER JOIN role_mapping AS rm
				ON n.id = rm.name_id
			INNER JOIN movie AS m
...
WINDOW w1 AS (ORDER BY ROUND(SUM(avg_rating * total_votes) / SUM(total_votes), 3) DESC);
...
SELECT name        AS actress_name,
       ...
       COUNT(title) AS movie_count,
       ROUND(SUM(avg_rating * total_votes) / SUM(total_votes), 3) AS actor_avg_rating,
       ROW_NUMBER() OVER w1 AS actress_rank
FROM   names AS n
		INNER JOIN role_mapping AS rm
			ON n.id = rm.name_id
		INNER JOIN movie AS m
...
WINDOW w1 AS (ORDER BY genre ROWS UNBOUNDED PRECEDING),
       w2 AS (ORDER BY genre ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW);
...
FROM   movie AS m
		...
		INNER JOIN top_3_genres AS t
			USING (genre)
WINDOW w1 AS (PARTITION BY year ORDER BY CAST(REPLACE(ifnull(worlwide_gross_income,0), '$', '') AS DECIMAL(10)) DESC)
)
SELECT *
FROM movie_summary
WHERE movie_rank <= 5
